...
class _ProductsByCategoryState extends State<ProductsByCategory> {
  ...
  void _fetchProducts() {
    futureProductsByCategory = GetProductsByCategory()
            .fetchProductByCategoryId(widget.categoryId, filters: _filters)
  ...
  void _showFilterDialog() async {
    final selectedFilters = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (context) => Filtering(
        initialSortOption: 'Relevance',
        initialPriceFilters: _filters['price'] ?? {},
        initialCategoryFilters: _filters['category'] ?? {},
        initialMaterialFilters: _filters['material'] ?? {},
        onFilterApplied: (filters) {
          setState(() {
            _filters = filters;
            _fetchProducts(); // Re-fetch products with new filters
          });
        },
      ),
    );
